...
export default function Dashboard() {
  ...
  // Auto VBN history
  type AutoVbnRun = { id: number; started_at: string; finished_at: string | null; checked_count: number | null; fixed_count: number | null; status: string; error: string | null; fixes: {product_id: string; name: string; old_vbn: string; new_vbn: string; ok: boolean}[] };
          ...
          {[
            ...
            <button
              ...
              onClick={() => {
                ...
                if (item.id === "history") { setHistorySubTab("ops"); loadHistory(); loadSyncHistory(); loadAutoVbnHistory(); }
              }}
            ...
            <div className="mb-5 bg-white border border-neutral-200 rounded-xl p-5 flex items-start gap-5">
              <div className="flex-1">
                ...
                {vbnAutoLastRun && (
                  <p className="text-xs text-neutral-400 mt-2">
                    {t.vbn.autoCheckLastRun}: {new Date(vbnAutoLastRun.started_at).toLocaleString(lang === "en" ? "en-GB" : lang === "nl" ? "nl-NL" : lang === "es" ? "es-ES" : "pl-PL")}
                ...
                {vbnAutoEnabled && vbnAutoNextRun && (
                  <p className="text-xs text-neutral-400 mt-0.5">
                    {t.vbn.autoCheckNextRun}: {new Date(vbnAutoNextRun).toLocaleString(lang === "en" ? "en-GB" : lang === "nl" ? "nl-NL" : lang === "es" ? "es-ES" : "pl-PL")}
                  </p>
                )}
              </div>
              {/* Toggle switch */}
              <button
                ...
                className={`relative inline-flex h-6 w-11 flex-shrink-0 items-center rounded-full transition-colors duration-200 focus:outline-none disabled:opacity-50 ${
                  vbnAutoEnabled ? "bg-violet-600" : "bg-neutral-200"
                }`}
                aria-label={t.vbn.autoCheckTitle}
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform duration-200 ${
                  vbnAutoEnabled ? "translate-x-6" : "translate-x-1"
                }`} />
              </button>
            </div>
            ...
            <div className="mb-6 flex justify-between items-start">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-0.5 bg-neutral-100 rounded-lg p-1">
                  ...
                  >
                    {t.history.subTabSync}
                  </button>
                  <button
                    onClick={() => setHistorySubTab("auto")}
                    className={`text-xs px-3 py-1.5 rounded-md font-medium transition-colors ${
                      historySubTab === "auto"
                        ? "bg-white text-neutral-800 shadow-sm"
                        : "text-neutral-500 hover:text-neutral-700"
                  ...
                  >
                    {t.history.subTabAutoVbn}
                  </button>
                </div>
                <button
                  onClick={() => historySubTab === "ops" ? loadHistory() : historySubTab === "sync" ? loadSyncHistory() : loadAutoVbnHistory()}
                          ...
                          <>
                            ...
                            >
                              ...
                              <span className="text-xs text-neutral-500 flex-shrink-0">
                                {new Date(run.started_at).toLocaleString(lang === "en" ? "en-GB" : lang === "nl" ? "nl-NL" : lang === "es" ? "es-ES" : "pl-PL")}
                              </span>
                              {dur !== null && (
            ...
            {historySubTab === "auto" && (
              <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
                {autoVbnHistLoading && autoVbnHistory === null ? (
                  ...
                  <>
                    <div className="divide-y divide-neutral-50">
                      {autoVbnHistory.map((run: AutoVbnRun) => {
