---
name: ship-to-main
description: Sends work from test_1 to main - the whole branch by merging, or only the ready part in a separate worktree - without disturbing unfinished work on test_1. Use only when the user explicitly asks for main ("pushuj na maina", "zmerguj test do maina", "wrzuc to na produkcje").
---
...
2. Bring the ready part over, either by `git cherry-pick <sha>` for whole
   commits that are ready as they are, or by making the change again as its
   own commit when only part of a commit is ready. On 2026-09-22 the Stock
   option went over this way as `049b6a5`, without the invoice picker it had
   been built with, because that API still needed rework.
3. Run the checks before pushing. **`.claude/` does not exist on `main`**, so
   `ship-to-test`'s `verify.py` is not there: copy it into the worktree and
   leave it uncommitted, or run it from the `test_1` checkout against the
   worktree's files. Never commit it to `main`.
4. `git push origin main`, then `git worktree remove ../main-ship`.
5. Report which commits went and which stayed.
...
- Commit `.claude/` or a copied `verify.py` to `main`.
